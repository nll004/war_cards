...
// ======================  PATCH /user/:username/stats Route ===============================================
describe('PATCH /users/:username/stats', function () {
    ...
    it("should successfully add user's stats to existing stats if valid users token", async function () {
        expect.assertions(4);
        const statsBeforeChange = await request(app).get('/users/testUser/stats').set('authorization', `Bearer ${nonAdminToken}`)
        expect(statsBeforeChange.body).toEqual({
            success: true,
            gameStats: {
                username: 'testUser',
                gamesPlayed: 5,
        ...
        const statsAfterChange = await request(app).get('/users/testUser/stats')
            .set('authorization', `Bearer ${nonAdminToken}`)
        expect(statsAfterChange.body).toEqual({
            success: true,
            gameStats: {
                username: 'testUser',
                gamesPlayed: 6,
